...
import (
	...
	"github.com/Thenecromance/Go_Blizzard_API/api/wow/ProfileService/CharacterProfile"
...
func ginwowCharacterProfileSummary(c *gin.Context) {
	...
	resp, err := wow_CharacterProfile.CharacterProfileSummary(c, &req)
	if err != nil {
...
func ginwowCharacterProfileStatus(c *gin.Context) {
	// binding uri parameters
	var req wow_CharacterProfile.CharacterProfileStatusFields
	if err := c.ShouldBindUri(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"msg": err.Error()})
		return
	}
	...
	resp, err := wow_CharacterProfile.CharacterProfileStatus(c, &req)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"msg": err.Error()})
		return
	}
