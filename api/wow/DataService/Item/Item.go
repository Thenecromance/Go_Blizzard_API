package wow_Item

// AUTOMATICALLY GENERATED FILE, DO NOT EDIT
// This file is generated by tools/updater
// Author: @Thenecromance

import (
	"context"
	"encoding/json"

	"strconv"

	"io"
	"net/http"

	"github.com/Thenecromance/Go_Blizzard_API/ApiError"
	"github.com/Thenecromance/Go_Blizzard_API/api/Authentication"
	"github.com/Thenecromance/Go_Blizzard_API/global"
	"github.com/Thenecromance/Go_Blizzard_API/utils"

	"github.com/jtacoma/uritemplates"
)

// ==============================================================================================
// API: Item
// ==============================================================================================

type ItemFields struct {
	ItemId    string `uri:"itemId" binding:"required"`    // The ID of the item.
	Namespace string `form:"namespace,default=static-us"` // The namespace to use to locate this document.
	Locale    string `form:"locale,default=en_US"`        // The locale to reflect in localized data.

	// Extra fields for internal logic
	ExtraFields map[any]any
	CN          *utils.CNRequestMethod
	utils.RequestMethod
}

// CNHookItem allows injecting a custom implementation for the CN endpoint.
// Implement this in a separate file (e.g. init()) to handle CN logic without modifying generated code.
var CNHookItem func(context.Context, *ItemFields) (any, error)

// StringItem executes the request and returns the raw response body string.
func StringItem(ctx context.Context, fields *ItemFields) (string, error) {
	if fields == nil {
		fields = &ItemFields{}
	}

	// 1. Initialize Request Metadata
	fields.RequestMethod = utils.GetApiConfig("Item")
	if fields.RequestMethod.Path == "" {
		fields.RequestMethod = utils.RequestMethod{
			Methods:  "GET",
			Path:     "/data/wow/item/{itemId}",
			CnRegion: false,
		}
	}

	// 2. Apply Default Values (if needed for client-side logic)
	// Note: Usually struct tags handle server-side binding,
	// but here we might need manual checks if 0/"" are invalid for the request.

	if fields.ItemId == "" {
		fields.ItemId = "19019"
	}

	if fields.Namespace == "" {
		fields.Namespace = "static-us"
	}

	if fields.Locale == "" {
		fields.Locale = "en_US"
	}

	// 3. Create HTTP Request
	req, err := http.NewRequestWithContext(
		ctx,
		fields.Methods,
		global.PortalURL, // Base URL will be joined with Path below
		nil,
	)
	if err != nil {
		return "", err
	}

	// 4. Resolve Path (Handle URI Bindings)
	{

		tpl, err := uritemplates.Parse(fields.Path)
		if err != nil {
			return "", err
		}

		pathValues := map[string]interface{}{
			"itemId": fields.ItemId,
		}

		expandedPath, err := tpl.Expand(pathValues)
		if err != nil {
			return "", err
		}
		req.URL.Path = expandedPath

	}

	// 5. Build Query Strings
	{
		q := req.URL.Query()

		for key, value := range fields.ExtraFields {
			q.Add(key.(string), value.(string))
		}

		if !q.Has("namespace") {
			q.Add("namespace", "static-us")
		}

		if !q.Has("locale") {
			q.Add("locale", "en_US")
		}

		req.URL.RawQuery = q.Encode()
	}

	// 6. Execute Request
	resp, err := Authentication.Client().Do(req)
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()

	responseBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		return "", err
	}

	return string(responseBytes), nil
}

// bridgeItem routes the request to either CN or Global logic based on input.
func bridgeItem(ctx context.Context, fields *ItemFields) (any, error) {

	// 1. If CN specific parameters are present, use CN logic
	if fields.CN != nil {
		// Design Scheme: Check if a custom CN handler is registered at runtime.
		// This allows extension without modifying the template generator.
		if CNHookItem != nil {
			return CNHookItem(ctx, fields)
		}

		return nil, ApiError.ErrorNotSupported
	}

	// 2. Default: Use Global/Standard logic
	objString, err := StringItem(ctx, fields)
	if err != nil {
		return nil, err
	}

	resp := &ItemModel{}
	if err = json.Unmarshal([]byte(objString), &resp); err != nil {
		return nil, err
	}

	return resp, nil
}

// Item
/* Item Returns an item by ID. */
// Path: /data/wow/item/{itemId}
var Item = bridgeItem

// ==============================================================================================
// API: ItemSearch
// ==============================================================================================

type ItemSearchFields struct {
	Namespace string `form:"namespace,default=static-us"` // The namespace to use to locate this document.
	Nameen_US string `form:"nameen_US,default=Garrosh"`   // The name of the item. (example search field)
	Orderby   string `form:"orderby,default=id"`          // The field to sort the result set by.
	_page     int    `form:"_page,default=1"`             // The result page number.

	// Extra fields for internal logic
	ExtraFields map[any]any
	CN          *utils.CNRequestMethod
	utils.RequestMethod
}

// CNHookItemSearch allows injecting a custom implementation for the CN endpoint.
// Implement this in a separate file (e.g. init()) to handle CN logic without modifying generated code.
var CNHookItemSearch func(context.Context, *ItemSearchFields) (any, error)

// StringItemSearch executes the request and returns the raw response body string.
func StringItemSearch(ctx context.Context, fields *ItemSearchFields) (string, error) {
	if fields == nil {
		fields = &ItemSearchFields{}
	}

	// 1. Initialize Request Metadata
	fields.RequestMethod = utils.GetApiConfig("ItemSearch")
	if fields.RequestMethod.Path == "" {
		fields.RequestMethod = utils.RequestMethod{
			Methods:  "GET",
			Path:     "/data/wow/search/item",
			CnRegion: false,
		}
	}

	// 2. Apply Default Values (if needed for client-side logic)
	// Note: Usually struct tags handle server-side binding,
	// but here we might need manual checks if 0/"" are invalid for the request.

	if fields.Namespace == "" {
		fields.Namespace = "static-us"
	}

	if fields.Nameen_US == "" {
		fields.Nameen_US = "Garrosh"
	}

	if fields.Orderby == "" {
		fields.Orderby = "id"
	}

	if fields._page == 0 {
		fields._page = 1
	}

	// 3. Create HTTP Request
	req, err := http.NewRequestWithContext(
		ctx,
		fields.Methods,
		global.PortalURL, // Base URL will be joined with Path below
		nil,
	)
	if err != nil {
		return "", err
	}

	// 4. Resolve Path (Handle URI Bindings)
	{

		req.URL.Path = fields.Path

	}

	// 5. Build Query Strings
	{
		q := req.URL.Query()

		for key, value := range fields.ExtraFields {
			q.Add(key.(string), value.(string))
		}

		if !q.Has("namespace") {
			q.Add("namespace", "static-us")
		}

		if !q.Has("nameen_US") {
			q.Add("nameen_US", "Garrosh")
		}

		if !q.Has("orderby") {
			q.Add("orderby", "id")
		}

		if !q.Has("_page") {
			q.Add("_page", strconv.Itoa(fields._page))
		}

		req.URL.RawQuery = q.Encode()
	}

	// 6. Execute Request
	resp, err := Authentication.Client().Do(req)
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()

	responseBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		return "", err
	}

	return string(responseBytes), nil
}

// bridgeItemSearch routes the request to either CN or Global logic based on input.
func bridgeItemSearch(ctx context.Context, fields *ItemSearchFields) (any, error) {

	// 1. If CN specific parameters are present, use CN logic
	if fields.CN != nil {
		// Design Scheme: Check if a custom CN handler is registered at runtime.
		// This allows extension without modifying the template generator.
		if CNHookItemSearch != nil {
			return CNHookItemSearch(ctx, fields)
		}

		return nil, ApiError.ErrorNotSupported
	}

	// 2. Default: Use Global/Standard logic
	objString, err := StringItemSearch(ctx, fields)
	if err != nil {
		return nil, err
	}

	resp := &ItemSearchModel{}
	if err = json.Unmarshal([]byte(objString), &resp); err != nil {
		return nil, err
	}

	return resp, nil
}

// ItemSearch
/* ItemSearch Performs a search of items. The fields below are provided for example. For more detail see the <a href="/documentation/world-of-warcraft/guides/search">Search Guide</a>. */
// Path: /data/wow/search/item
var ItemSearch = bridgeItemSearch

// ==============================================================================================
// API: ItemMedia
// ==============================================================================================

type ItemMediaFields struct {
	ItemId    int    `uri:"itemId" binding:"required"`    // The ID of the item.
	Namespace string `form:"namespace,default=static-us"` // The namespace to use to locate this document.
	Locale    string `form:"locale,default=en_US"`        // The locale to reflect in localized data.

	// Extra fields for internal logic
	ExtraFields map[any]any
	CN          *utils.CNRequestMethod
	utils.RequestMethod
}

// CNHookItemMedia allows injecting a custom implementation for the CN endpoint.
// Implement this in a separate file (e.g. init()) to handle CN logic without modifying generated code.
var CNHookItemMedia func(context.Context, *ItemMediaFields) (any, error)

// StringItemMedia executes the request and returns the raw response body string.
func StringItemMedia(ctx context.Context, fields *ItemMediaFields) (string, error) {
	if fields == nil {
		fields = &ItemMediaFields{}
	}

	// 1. Initialize Request Metadata
	fields.RequestMethod = utils.GetApiConfig("ItemMedia")
	if fields.RequestMethod.Path == "" {
		fields.RequestMethod = utils.RequestMethod{
			Methods:  "GET",
			Path:     "/data/wow/media/item/{itemId}",
			CnRegion: false,
		}
	}

	// 2. Apply Default Values (if needed for client-side logic)
	// Note: Usually struct tags handle server-side binding,
	// but here we might need manual checks if 0/"" are invalid for the request.

	if fields.ItemId == 0 {
		fields.ItemId = 19019
	}

	if fields.Namespace == "" {
		fields.Namespace = "static-us"
	}

	if fields.Locale == "" {
		fields.Locale = "en_US"
	}

	// 3. Create HTTP Request
	req, err := http.NewRequestWithContext(
		ctx,
		fields.Methods,
		global.PortalURL, // Base URL will be joined with Path below
		nil,
	)
	if err != nil {
		return "", err
	}

	// 4. Resolve Path (Handle URI Bindings)
	{

		tpl, err := uritemplates.Parse(fields.Path)
		if err != nil {
			return "", err
		}

		pathValues := map[string]interface{}{
			"itemId": fields.ItemId,
		}

		expandedPath, err := tpl.Expand(pathValues)
		if err != nil {
			return "", err
		}
		req.URL.Path = expandedPath

	}

	// 5. Build Query Strings
	{
		q := req.URL.Query()

		for key, value := range fields.ExtraFields {
			q.Add(key.(string), value.(string))
		}

		if !q.Has("namespace") {
			q.Add("namespace", "static-us")
		}

		if !q.Has("locale") {
			q.Add("locale", "en_US")
		}

		req.URL.RawQuery = q.Encode()
	}

	// 6. Execute Request
	resp, err := Authentication.Client().Do(req)
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()

	responseBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		return "", err
	}

	return string(responseBytes), nil
}

// bridgeItemMedia routes the request to either CN or Global logic based on input.
func bridgeItemMedia(ctx context.Context, fields *ItemMediaFields) (any, error) {

	// 1. If CN specific parameters are present, use CN logic
	if fields.CN != nil {
		// Design Scheme: Check if a custom CN handler is registered at runtime.
		// This allows extension without modifying the template generator.
		if CNHookItemMedia != nil {
			return CNHookItemMedia(ctx, fields)
		}

		return nil, ApiError.ErrorNotSupported
	}

	// 2. Default: Use Global/Standard logic
	objString, err := StringItemMedia(ctx, fields)
	if err != nil {
		return nil, err
	}

	resp := &ItemMediaModel{}
	if err = json.Unmarshal([]byte(objString), &resp); err != nil {
		return nil, err
	}

	return resp, nil
}

// ItemMedia
/* ItemMedia Returns media for an item by ID. */
// Path: /data/wow/media/item/{itemId}
var ItemMedia = bridgeItemMedia

// ==============================================================================================
// API: ItemClassesIndex
// ==============================================================================================

type ItemClassesIndexFields struct {
	Namespace string `form:"namespace,default=static-us"` // The namespace to use to locate this document.
	Locale    string `form:"locale,default=en_US"`        // The locale to reflect in localized data.

	// Extra fields for internal logic
	ExtraFields map[any]any
	CN          *utils.CNRequestMethod
	utils.RequestMethod
}

// CNHookItemClassesIndex allows injecting a custom implementation for the CN endpoint.
// Implement this in a separate file (e.g. init()) to handle CN logic without modifying generated code.
var CNHookItemClassesIndex func(context.Context, *ItemClassesIndexFields) (any, error)

// StringItemClassesIndex executes the request and returns the raw response body string.
func StringItemClassesIndex(ctx context.Context, fields *ItemClassesIndexFields) (string, error) {
	if fields == nil {
		fields = &ItemClassesIndexFields{}
	}

	// 1. Initialize Request Metadata
	fields.RequestMethod = utils.GetApiConfig("ItemClassesIndex")
	if fields.RequestMethod.Path == "" {
		fields.RequestMethod = utils.RequestMethod{
			Methods:  "GET",
			Path:     "/data/wow/item-class/index",
			CnRegion: false,
		}
	}

	// 2. Apply Default Values (if needed for client-side logic)
	// Note: Usually struct tags handle server-side binding,
	// but here we might need manual checks if 0/"" are invalid for the request.

	if fields.Namespace == "" {
		fields.Namespace = "static-us"
	}

	if fields.Locale == "" {
		fields.Locale = "en_US"
	}

	// 3. Create HTTP Request
	req, err := http.NewRequestWithContext(
		ctx,
		fields.Methods,
		global.PortalURL, // Base URL will be joined with Path below
		nil,
	)
	if err != nil {
		return "", err
	}

	// 4. Resolve Path (Handle URI Bindings)
	{

		req.URL.Path = fields.Path

	}

	// 5. Build Query Strings
	{
		q := req.URL.Query()

		for key, value := range fields.ExtraFields {
			q.Add(key.(string), value.(string))
		}

		if !q.Has("namespace") {
			q.Add("namespace", "static-us")
		}

		if !q.Has("locale") {
			q.Add("locale", "en_US")
		}

		req.URL.RawQuery = q.Encode()
	}

	// 6. Execute Request
	resp, err := Authentication.Client().Do(req)
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()

	responseBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		return "", err
	}

	return string(responseBytes), nil
}

// bridgeItemClassesIndex routes the request to either CN or Global logic based on input.
func bridgeItemClassesIndex(ctx context.Context, fields *ItemClassesIndexFields) (any, error) {

	// 1. If CN specific parameters are present, use CN logic
	if fields.CN != nil {
		// Design Scheme: Check if a custom CN handler is registered at runtime.
		// This allows extension without modifying the template generator.
		if CNHookItemClassesIndex != nil {
			return CNHookItemClassesIndex(ctx, fields)
		}

		return nil, ApiError.ErrorNotSupported
	}

	// 2. Default: Use Global/Standard logic
	objString, err := StringItemClassesIndex(ctx, fields)
	if err != nil {
		return nil, err
	}

	resp := &ItemClassesIndexModel{}
	if err = json.Unmarshal([]byte(objString), &resp); err != nil {
		return nil, err
	}

	return resp, nil
}

// ItemClassesIndex
/* ItemClassesIndex Returns an index of item classes. */
// Path: /data/wow/item-class/index
var ItemClassesIndex = bridgeItemClassesIndex

// ==============================================================================================
// API: ItemClass
// ==============================================================================================

type ItemClassFields struct {
	ItemClassId string `uri:"itemClassId" binding:"required"` // The ID of the item class.
	Namespace   string `form:"namespace,default=static-us"`   // The namespace to use to locate this document.
	Locale      string `form:"locale,default=en_US"`          // The locale to reflect in localized data.

	// Extra fields for internal logic
	ExtraFields map[any]any
	CN          *utils.CNRequestMethod
	utils.RequestMethod
}

// CNHookItemClass allows injecting a custom implementation for the CN endpoint.
// Implement this in a separate file (e.g. init()) to handle CN logic without modifying generated code.
var CNHookItemClass func(context.Context, *ItemClassFields) (any, error)

// StringItemClass executes the request and returns the raw response body string.
func StringItemClass(ctx context.Context, fields *ItemClassFields) (string, error) {
	if fields == nil {
		fields = &ItemClassFields{}
	}

	// 1. Initialize Request Metadata
	fields.RequestMethod = utils.GetApiConfig("ItemClass")
	if fields.RequestMethod.Path == "" {
		fields.RequestMethod = utils.RequestMethod{
			Methods:  "GET",
			Path:     "/data/wow/item-class/{itemClassId}",
			CnRegion: false,
		}
	}

	// 2. Apply Default Values (if needed for client-side logic)
	// Note: Usually struct tags handle server-side binding,
	// but here we might need manual checks if 0/"" are invalid for the request.

	if fields.ItemClassId == "" {
		fields.ItemClassId = "0"
	}

	if fields.Namespace == "" {
		fields.Namespace = "static-us"
	}

	if fields.Locale == "" {
		fields.Locale = "en_US"
	}

	// 3. Create HTTP Request
	req, err := http.NewRequestWithContext(
		ctx,
		fields.Methods,
		global.PortalURL, // Base URL will be joined with Path below
		nil,
	)
	if err != nil {
		return "", err
	}

	// 4. Resolve Path (Handle URI Bindings)
	{

		tpl, err := uritemplates.Parse(fields.Path)
		if err != nil {
			return "", err
		}

		pathValues := map[string]interface{}{
			"itemClassId": fields.ItemClassId,
		}

		expandedPath, err := tpl.Expand(pathValues)
		if err != nil {
			return "", err
		}
		req.URL.Path = expandedPath

	}

	// 5. Build Query Strings
	{
		q := req.URL.Query()

		for key, value := range fields.ExtraFields {
			q.Add(key.(string), value.(string))
		}

		if !q.Has("namespace") {
			q.Add("namespace", "static-us")
		}

		if !q.Has("locale") {
			q.Add("locale", "en_US")
		}

		req.URL.RawQuery = q.Encode()
	}

	// 6. Execute Request
	resp, err := Authentication.Client().Do(req)
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()

	responseBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		return "", err
	}

	return string(responseBytes), nil
}

// bridgeItemClass routes the request to either CN or Global logic based on input.
func bridgeItemClass(ctx context.Context, fields *ItemClassFields) (any, error) {

	// 1. If CN specific parameters are present, use CN logic
	if fields.CN != nil {
		// Design Scheme: Check if a custom CN handler is registered at runtime.
		// This allows extension without modifying the template generator.
		if CNHookItemClass != nil {
			return CNHookItemClass(ctx, fields)
		}

		return nil, ApiError.ErrorNotSupported
	}

	// 2. Default: Use Global/Standard logic
	objString, err := StringItemClass(ctx, fields)
	if err != nil {
		return nil, err
	}

	resp := &ItemClassModel{}
	if err = json.Unmarshal([]byte(objString), &resp); err != nil {
		return nil, err
	}

	return resp, nil
}

// ItemClass
/* ItemClass Returns an item class by ID. */
// Path: /data/wow/item-class/{itemClassId}
var ItemClass = bridgeItemClass

// ==============================================================================================
// API: ItemSetsIndex
// ==============================================================================================

type ItemSetsIndexFields struct {
	Namespace string `form:"namespace,default=static-us"` // The namespace to use to locate this document.
	Locale    string `form:"locale,default=en_US"`        // The locale to reflect in localized data.

	// Extra fields for internal logic
	ExtraFields map[any]any
	CN          *utils.CNRequestMethod
	utils.RequestMethod
}

// CNHookItemSetsIndex allows injecting a custom implementation for the CN endpoint.
// Implement this in a separate file (e.g. init()) to handle CN logic without modifying generated code.
var CNHookItemSetsIndex func(context.Context, *ItemSetsIndexFields) (any, error)

// StringItemSetsIndex executes the request and returns the raw response body string.
func StringItemSetsIndex(ctx context.Context, fields *ItemSetsIndexFields) (string, error) {
	if fields == nil {
		fields = &ItemSetsIndexFields{}
	}

	// 1. Initialize Request Metadata
	fields.RequestMethod = utils.GetApiConfig("ItemSetsIndex")
	if fields.RequestMethod.Path == "" {
		fields.RequestMethod = utils.RequestMethod{
			Methods:  "GET",
			Path:     "/data/wow/item-set/index",
			CnRegion: false,
		}
	}

	// 2. Apply Default Values (if needed for client-side logic)
	// Note: Usually struct tags handle server-side binding,
	// but here we might need manual checks if 0/"" are invalid for the request.

	if fields.Namespace == "" {
		fields.Namespace = "static-us"
	}

	if fields.Locale == "" {
		fields.Locale = "en_US"
	}

	// 3. Create HTTP Request
	req, err := http.NewRequestWithContext(
		ctx,
		fields.Methods,
		global.PortalURL, // Base URL will be joined with Path below
		nil,
	)
	if err != nil {
		return "", err
	}

	// 4. Resolve Path (Handle URI Bindings)
	{

		req.URL.Path = fields.Path

	}

	// 5. Build Query Strings
	{
		q := req.URL.Query()

		for key, value := range fields.ExtraFields {
			q.Add(key.(string), value.(string))
		}

		if !q.Has("namespace") {
			q.Add("namespace", "static-us")
		}

		if !q.Has("locale") {
			q.Add("locale", "en_US")
		}

		req.URL.RawQuery = q.Encode()
	}

	// 6. Execute Request
	resp, err := Authentication.Client().Do(req)
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()

	responseBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		return "", err
	}

	return string(responseBytes), nil
}

// bridgeItemSetsIndex routes the request to either CN or Global logic based on input.
func bridgeItemSetsIndex(ctx context.Context, fields *ItemSetsIndexFields) (any, error) {

	// 1. If CN specific parameters are present, use CN logic
	if fields.CN != nil {
		// Design Scheme: Check if a custom CN handler is registered at runtime.
		// This allows extension without modifying the template generator.
		if CNHookItemSetsIndex != nil {
			return CNHookItemSetsIndex(ctx, fields)
		}

		return nil, ApiError.ErrorNotSupported
	}

	// 2. Default: Use Global/Standard logic
	objString, err := StringItemSetsIndex(ctx, fields)
	if err != nil {
		return nil, err
	}

	resp := &ItemSetsIndexModel{}
	if err = json.Unmarshal([]byte(objString), &resp); err != nil {
		return nil, err
	}

	return resp, nil
}

// ItemSetsIndex
/* ItemSetsIndex Returns an index of item sets. */
// Path: /data/wow/item-set/index
var ItemSetsIndex = bridgeItemSetsIndex

// ==============================================================================================
// API: ItemSet
// ==============================================================================================

type ItemSetFields struct {
	ItemSetId int    `uri:"itemSetId" binding:"required"` // The ID of the item set.
	Namespace string `form:"namespace,default=static-us"` // The namespace to use to locate this document.
	Locale    string `form:"locale,default=en_US"`        // The locale to reflect in localized data.

	// Extra fields for internal logic
	ExtraFields map[any]any
	CN          *utils.CNRequestMethod
	utils.RequestMethod
}

// CNHookItemSet allows injecting a custom implementation for the CN endpoint.
// Implement this in a separate file (e.g. init()) to handle CN logic without modifying generated code.
var CNHookItemSet func(context.Context, *ItemSetFields) (any, error)

// StringItemSet executes the request and returns the raw response body string.
func StringItemSet(ctx context.Context, fields *ItemSetFields) (string, error) {
	if fields == nil {
		fields = &ItemSetFields{}
	}

	// 1. Initialize Request Metadata
	fields.RequestMethod = utils.GetApiConfig("ItemSet")
	if fields.RequestMethod.Path == "" {
		fields.RequestMethod = utils.RequestMethod{
			Methods:  "GET",
			Path:     "/data/wow/item-set/{itemSetId}",
			CnRegion: false,
		}
	}

	// 2. Apply Default Values (if needed for client-side logic)
	// Note: Usually struct tags handle server-side binding,
	// but here we might need manual checks if 0/"" are invalid for the request.

	if fields.ItemSetId == 0 {
		fields.ItemSetId = 1
	}

	if fields.Namespace == "" {
		fields.Namespace = "static-us"
	}

	if fields.Locale == "" {
		fields.Locale = "en_US"
	}

	// 3. Create HTTP Request
	req, err := http.NewRequestWithContext(
		ctx,
		fields.Methods,
		global.PortalURL, // Base URL will be joined with Path below
		nil,
	)
	if err != nil {
		return "", err
	}

	// 4. Resolve Path (Handle URI Bindings)
	{

		tpl, err := uritemplates.Parse(fields.Path)
		if err != nil {
			return "", err
		}

		pathValues := map[string]interface{}{
			"itemSetId": fields.ItemSetId,
		}

		expandedPath, err := tpl.Expand(pathValues)
		if err != nil {
			return "", err
		}
		req.URL.Path = expandedPath

	}

	// 5. Build Query Strings
	{
		q := req.URL.Query()

		for key, value := range fields.ExtraFields {
			q.Add(key.(string), value.(string))
		}

		if !q.Has("namespace") {
			q.Add("namespace", "static-us")
		}

		if !q.Has("locale") {
			q.Add("locale", "en_US")
		}

		req.URL.RawQuery = q.Encode()
	}

	// 6. Execute Request
	resp, err := Authentication.Client().Do(req)
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()

	responseBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		return "", err
	}

	return string(responseBytes), nil
}

// bridgeItemSet routes the request to either CN or Global logic based on input.
func bridgeItemSet(ctx context.Context, fields *ItemSetFields) (any, error) {

	// 1. If CN specific parameters are present, use CN logic
	if fields.CN != nil {
		// Design Scheme: Check if a custom CN handler is registered at runtime.
		// This allows extension without modifying the template generator.
		if CNHookItemSet != nil {
			return CNHookItemSet(ctx, fields)
		}

		return nil, ApiError.ErrorNotSupported
	}

	// 2. Default: Use Global/Standard logic
	objString, err := StringItemSet(ctx, fields)
	if err != nil {
		return nil, err
	}

	resp := &ItemSetModel{}
	if err = json.Unmarshal([]byte(objString), &resp); err != nil {
		return nil, err
	}

	return resp, nil
}

// ItemSet
/* ItemSet Returns an item set by ID. */
// Path: /data/wow/item-set/{itemSetId}
var ItemSet = bridgeItemSet

// ==============================================================================================
// API: ItemSubclass
// ==============================================================================================

type ItemSubclassFields struct {
	ItemClassId    string `uri:"itemClassId" binding:"required"`    // The ID of the item class.
	ItemSubclassId string `uri:"itemSubclassId" binding:"required"` // The ID of the item subclass.
	Namespace      string `form:"namespace,default=static-us"`      // The namespace to use to locate this document.
	Locale         string `form:"locale,default=en_US"`             // The locale to reflect in localized data.

	// Extra fields for internal logic
	ExtraFields map[any]any
	CN          *utils.CNRequestMethod
	utils.RequestMethod
}

// CNHookItemSubclass allows injecting a custom implementation for the CN endpoint.
// Implement this in a separate file (e.g. init()) to handle CN logic without modifying generated code.
var CNHookItemSubclass func(context.Context, *ItemSubclassFields) (any, error)

// StringItemSubclass executes the request and returns the raw response body string.
func StringItemSubclass(ctx context.Context, fields *ItemSubclassFields) (string, error) {
	if fields == nil {
		fields = &ItemSubclassFields{}
	}

	// 1. Initialize Request Metadata
	fields.RequestMethod = utils.GetApiConfig("ItemSubclass")
	if fields.RequestMethod.Path == "" {
		fields.RequestMethod = utils.RequestMethod{
			Methods:  "GET",
			Path:     "/data/wow/item-class/{itemClassId}/item-subclass/{itemSubclassId}",
			CnRegion: false,
		}
	}

	// 2. Apply Default Values (if needed for client-side logic)
	// Note: Usually struct tags handle server-side binding,
	// but here we might need manual checks if 0/"" are invalid for the request.

	if fields.ItemClassId == "" {
		fields.ItemClassId = "0"
	}

	if fields.ItemSubclassId == "" {
		fields.ItemSubclassId = "0"
	}

	if fields.Namespace == "" {
		fields.Namespace = "static-us"
	}

	if fields.Locale == "" {
		fields.Locale = "en_US"
	}

	// 3. Create HTTP Request
	req, err := http.NewRequestWithContext(
		ctx,
		fields.Methods,
		global.PortalURL, // Base URL will be joined with Path below
		nil,
	)
	if err != nil {
		return "", err
	}

	// 4. Resolve Path (Handle URI Bindings)
	{

		tpl, err := uritemplates.Parse(fields.Path)
		if err != nil {
			return "", err
		}

		pathValues := map[string]interface{}{
			"itemClassId":    fields.ItemClassId,
			"itemSubclassId": fields.ItemSubclassId,
		}

		expandedPath, err := tpl.Expand(pathValues)
		if err != nil {
			return "", err
		}
		req.URL.Path = expandedPath

	}

	// 5. Build Query Strings
	{
		q := req.URL.Query()

		for key, value := range fields.ExtraFields {
			q.Add(key.(string), value.(string))
		}

		if !q.Has("namespace") {
			q.Add("namespace", "static-us")
		}

		if !q.Has("locale") {
			q.Add("locale", "en_US")
		}

		req.URL.RawQuery = q.Encode()
	}

	// 6. Execute Request
	resp, err := Authentication.Client().Do(req)
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()

	responseBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		return "", err
	}

	return string(responseBytes), nil
}

// bridgeItemSubclass routes the request to either CN or Global logic based on input.
func bridgeItemSubclass(ctx context.Context, fields *ItemSubclassFields) (any, error) {

	// 1. If CN specific parameters are present, use CN logic
	if fields.CN != nil {
		// Design Scheme: Check if a custom CN handler is registered at runtime.
		// This allows extension without modifying the template generator.
		if CNHookItemSubclass != nil {
			return CNHookItemSubclass(ctx, fields)
		}

		return nil, ApiError.ErrorNotSupported
	}

	// 2. Default: Use Global/Standard logic
	objString, err := StringItemSubclass(ctx, fields)
	if err != nil {
		return nil, err
	}

	resp := &ItemSubclassModel{}
	if err = json.Unmarshal([]byte(objString), &resp); err != nil {
		return nil, err
	}

	return resp, nil
}

// ItemSubclass
/* ItemSubclass Returns an item subclass by ID. */
// Path: /data/wow/item-class/{itemClassId}/item-subclass/{itemSubclassId}
var ItemSubclass = bridgeItemSubclass
